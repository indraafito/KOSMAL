  ...
  return (
    <div className="flex min-h-[calc(100vh-7rem)] items-center justify-center bg-gradient-hero px-4 py-12">
      <div className="w-full max-w-md rounded-2xl border border-border bg-card p-8 shadow-glow">
        ...
        <h1 className="text-center font-display text-2xl font-extrabold text-foreground">Daftar KOSMAL</h1>
        <p className="mt-1 text-center text-sm text-muted-foreground">Gratis tanpa drama, mulai cari kos sekarang.</p>
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          <div>
            ...
            <Input id="fullName" required value={fullName} onChange={(e) => setFullName(e.target.value)} className="mt-1" placeholder="Nama lengkap kamu" />
          </div>
          <div>
            <Label htmlFor="phone">Nomor HP</Label>
            <Input id="phone" type="tel" required value={phone} onChange={(e) => setPhone(e.target.value)} className="mt-1" placeholder="081234567890" />
          </div>
          <div>
          ...
          <div>
            ...
            <PasswordInput id="password" required value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1" placeholder="Minimal 6 karakter" />
          </div>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          Sudah punya akun? <Link to="/login" className="font-semibold text-primary hover:underline">Masuk sekarang</Link>
        </p>
      </div>
      ...
      <Dialog open={verifyOpen} onOpenChange={setVerifyOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Kami telah mengirim email verifikasi ke <strong>{email}</strong>. Klik link di email tersebut untuk mengaktifkan akun kamu.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button onClick={() => setVerifyOpen(false)} className="bg-gradient-cta">
              Mengerti
            </Button>
          </DialogFooter>
        </DialogContent>
